...
con.data<-read.csv("use_for_pub_conif_spcies_fire_master_forhisse.csv",row.names=1)

#set your trait via column names in csv file
hd<-data.frame(taxa=rownames(con.data), pyro=con.data[,"Resprout"])
head(hd)
...
write.csv(master.rates.table.new, "More_March_25_2023.csv")

write.csv(bark.dataframe, "More_dataframe_25032023.csv")
